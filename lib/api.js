...
async function fetchAll(config) {
  ...
  const limit = Math.min(first.totalCount, config.maxRecords);
  const totalPages = Math.ceil(limit / config.numOfRows);

  for (let page = 2; page <= totalPages; page += 1) {
    const next = await fetchPage(config, page);
  ...
  return {
    ...
    fetchedCount: Math.min(items.length, limit),
    format: first.format,
    requestedType: first.requestedType,
    truncated: first.totalCount > limit,
  };
}
