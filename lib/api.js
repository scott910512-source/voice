...
async function fetchAll(config) {
  ...
  // 이 API는 totalCount로 111을 알려주지만 pageNo를 올려도 같은 레코드를
  // 다시 돌려준다. 중복을 걸러내지 않으면 같은 표지가 여러 개 쌓인다.
  const seen = new Set();
  const items = [];
  const addNew = (records) => {
    let added = 0;
  ...
  for (let page = 2; items.length < limit && page <= maxPages; page += 1) {
    ...
    const added = addNew(next.items);
    if (added === 0) {
      notes.push(`${page}페이지가 1페이지와 같은 레코드만 돌려줌 (서버 페이징 미동작). 고유 ${items.length}건에서 중단`);
      break;
    }
  }
  ...
  return {
    ...
    // 서버가 알려준 전체 건수보다 고유 레코드가 적으면 그만큼 못 받은 것이다.
